...
but in practice **returns diminish well before that limit**: performance
plateaus at roughly **4-8 threads** (~2× speedup). Additional threads beyond
...
~40 minutes (single-threaded) to ~19 minutes (2.1× speedup). Beyond 8
threads, gains plateau -- additional threads are used mainly for BAM
decompression, yielding diminishing returns. Peak RSS grows linearly with
thread count as each worker maintains its own per-chromosome accumulators.
