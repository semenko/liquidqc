...
const DUPRADAR: Citation = Citation {
    ...
    description: "liquidqc replicates the duplication rate analysis of dupRadar.",
...
const FEATURECOUNTS: Citation = Citation {
    ...
    description: "liquidqc produces featureCounts-compatible gene-level read counts.",
...
const RSEQC: Citation = Citation {
    ...
    description: "liquidqc reimplements eight RSeQC quality control tools.",
...
const PRESEQ: Citation = Citation {
    ...
    description: "liquidqc reimplements library complexity estimation from Preseq.",
...
const SAMTOOLS: Citation = Citation {
    ...
    description: "liquidqc produces Samtools-compatible flagstat, idxstats, and stats output.",
...
const QUALIMAP: Citation = Citation {
    ...
    description: "liquidqc produces gene body coverage output compatible with Qualimap rnaseq.",
...
pub fn write_citations(path: &Path, config: &RnaConfig, version: &str, commit: &str) -> Result<()> {
    ...
    writeln!(w, "# liquidqc Citations\n")?;
    writeln!(
        w,
        "This file was generated by [liquidqc](https://github.com/semenko/liquidqc) v{version} ({commit}), \
         a fork of [RustQC](https://github.com/seqeralabs/RustQC) extended for cfRNA QC + fragmentomics."
    )?;
    writeln!(
        w,
    ...
    writeln!(
        w,
        "Please cite both liquidqc and the relevant upstream tools listed below.\n"
    )?;
    writeln!(w, "## liquidqc (v{version})\n")?;
    writeln!(w, "- Repository: <https://github.com/semenko/liquidqc>")?;
    writeln!(w, "- Forked from: <https://github.com/seqeralabs/RustQC>\n")?;
...
mod tests {
    ...
    fn default_config_includes_all_tools() {
        ...
        assert!(content.starts_with("# liquidqc Citations"));
    ...
    fn all_disabled_only_header() {
        ...
        assert!(content.contains("# liquidqc Citations"));
        assert!(content.contains("## liquidqc"));
        assert!(!content.contains("## dupRadar"));
        assert!(!content.contains("## featureCounts"));
        assert!(!content.contains("## RSeQC"));
        assert!(!content.contains("## Preseq"));
